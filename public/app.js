...
function handleCsvUpload(event) {
  ...
  reader.onload = function(e) {
    ...
    if (phoneNumbers.length > 0) {
      ...
      showError('Could not find phone numbers in the CSV file. Make sure there is a column with phone numbers.');
    }
  };
...
function parseCsv(csvContent) {
  ...
  const firstLineIsHeader = isNaN(firstLine.replace(/[+\-\s()]/g, '')) && 
  ...
  if (firstLineIsHeader) {
    ...
    phoneColumnIndex = header.findIndex(h => 
      ['phone', 'phonenumber', 'phone_number', 'number', 'mobile', 'telephone', 'tel', 'contact'].includes(h)
    );
...
function renderResultsList(results, filter) {
  ...
  if (filteredResults.length === 0) {
    resultsList.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">No results to display</p>';
    return;
  }
  ...
  const resultsHtml = filteredResults.map(result => {
    ...
    if (result.exists) {
      ...
      if (details.length > 0) {
        detailsHtml = `<div class="result-details">${details.join(' • ')}</div>`;
      }
